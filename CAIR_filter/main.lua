...
-- CAIR credentials
SendingFacility = "You sending facility ID"
...
function main(Data)
   local Msg, Name = hl7.parse{vmd="vxu.vmd", data=Data}
   local OutMsg = hl7.message{vmd="vxu.vmd", name=Name}
   OutMsg:mapTree(Msg)
   ...
   Xml["soap:Envelope"]["soap:Body"]["urn:submitSingleMessage"]["urn:hl7Message"][1] = OutMsg:S()
   Xml["soap:Envelope"]["soap:Body"]["urn:submitSingleMessage"]["urn:username"]:setInner(SendingFacility)
   Xml["soap:Envelope"]["soap:Body"]["urn:submitSingleMessage"]["urn:password"]:setInner(CairPassword)
